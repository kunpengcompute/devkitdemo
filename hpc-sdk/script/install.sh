...
install_hyper_mpi() {
  ...
  if [[ ${hmpi_gcc_choose_status} == 1 ]]; then
    ...
    sed -i "#hyper_mpi_gcc=.*#hyper_mpi_gcc=${compiler_name}#g" ${current_dir}/configure_environment.sh
    sed -i "#hyper_mpi_gcc_install=.*#hyper_mpi_gcc_install=$PWD#g"  ${current_dir}/configure_environment.sh
  elif [[ ${hmpi_bisheng_choose_status} == 1 ]]; then
    change_modules "HMPI-BISHENG"
    sed -i "#hyper_mpi_bisheng=.*#hyper_mpi_bisheng=${compiler_name}#g" ${current_dir}/configure_environment.sh
    sed -i "#hyper_mpi_bisheng_install=.*#hyper_mpi_bisheng_install=$PWD#g"  ${current_dir}/configure_environment.sh
  fi
...
install_compiler() {
  ...
  if [[ ${compiler_type} == "bisheng" ]]; then
    ...
    sed -i "#compiler_bisheng=.*#compiler_bisheng=${compiler_name}#g" ${current_dir}/configure_environment.sh
    sed -i "#compiler_bisheng_install=.*#compiler_bisheng_install=${install_compiler_path}#g"  ${current_dir}/configure_environment.sh
  else
    change_modules "GCC"
    sed -i "#compiler_gcc=.*#compiler_gcc=${compiler_name}#g" ${current_dir}/configure_environment.sh
    sed -i "#compiler_gcc_install=.*#compiler_gcc_install=${install_compiler_path}#g" ${current_dir}/configure_environment.sh
  fi
  if [[ ${compiler_type} == "gcc" ]]; then
